...
export const getFileContents = (fileName: string): Promise<string> => {
  return new Promise((resolve, rejects) => {
    ...
    const areas: {
      ...
      header: ["---", headerTitle, headerDescription, headerDate, "---"],
      toc,
      content: [],
      history,
    }
    ...
    rl.on("close", function () {
      if (currentLineIsHistory)
        throw new Error(
          "gitのコミット履歴を書き出すために使用している行が不正です。コミット履歴から作り直すのでコミット履歴用の行を削除してください"
        )
      if (currentLineIsToc)
        throw new Error(
          "tocを書き出すために使用している行が不正です。作り直すので、toc用の行を削除してください"
        )
      const header =
        areas.header.length === 0 ? "" : areas.header.join("\n") + "\n"
      const toc = areas.toc.length === 0 ? "" : areas.toc
      const content =
        areas.content.length === 0 ? "" : areas.content.join("\n") + "\n"
